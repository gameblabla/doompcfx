...
/* PC-FX: the framebuffer lives in KING KRAM, reached via the KING data port.
 * Drawers write pixels directly there (FB_col/FB_span/FB_put) — no RAM buffer. */
#include "pcfx_kram.h"
...
// KRAM row stride is 128 words (256px / 2 px-per-word). Framebuffer addressing is
// in KRAM words now, so a row step is <<7 = *128 (see pcfx_kram.h / FB_YOFFSET).
#define ScreenYToOffset(x) ((unsigned)(x) << 7)
