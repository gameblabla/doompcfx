...
#define ST_HEIGHT 32
#define ST_WIDTH  320
// PC-FX: status bar sits at the bottom of the full SCREENHEIGHT (was hardcoded
// 160). This places the HUD widgets (numbers/face) on the bar, not mid-screen.
...
// PC-FX: the 240px-wide GBADoom bar (STBAR.png / STBARFX) is padded to the 256px
// screen with an 8px black bar on the left, so every widget position (all of
// which are written as ST_X+n) is shifted right by 8px to land on the printed
// labels. See tools/gen_pcfx_hud.py and doom-pcfx/plan.md task #12.
#define ST_X                    8
#define ST_X2                   104
...
// PC-FX: the recessed face slot in STBAR.png sits at x=116..141, y=1 (measured),
// which is +4 past the uniform +8 shift, so the face gets its own X. The 24x29
// face graphic then fills the 26px slot. Nudged -5 X / -2 Y to seat it in-slot.
