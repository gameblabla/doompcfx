...
typedef struct
{
  ...
  // pointer to current value (points at an int; must NOT be short* — a 16-bit
  // deref reads the wrong half on the big-endian SH-1, so every value <=65535
  // reads as 0. GBADoom used short* and only worked because the GBA is LE.)
  int*  num;
...
// PC-FX full-resolution HUD compositor (see st_lib.c). Widgets composite into a
// 256x32 RAM strip; HUD_BlitStrip() flushes it to the bottom KRAM rows.
