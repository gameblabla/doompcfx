...
// PC-FX: the framebuffer is 8bpp but written as 16-bit KRAM words where each word
// paints two pixels (index | index<<8). So the colormap stores each entry PRE-
// DOUBLED as that 16-bit fat pixel (built in R_InitColormaps): the per-pixel inner
// loops then do a single `ld.h colormap[texel]` + store, dropping the `andi 255`
// + the `mov;shl 8;or` fat-pixel build that a byte colormap forces on every pixel.
typedef unsigned short  lighttable_t;
...
typedef struct vissprite_s
{
  ...
  const patch_t* patch;
  int lump;                    // sprite lump number, to re-cache patch at draw
                               // time (the patch may be evicted from PU_CACHE
                               // between BSP projection and the masked draw pass)
