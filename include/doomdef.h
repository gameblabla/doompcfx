...
// PC-FX: 128 logical columns (fat-pixel -> 256 screen px) x 240 lines, filling
// the KING 256x240 plane. In-game the 3D view is 240-32 = 208 lines above the
// 32-line status bar (ST_HEIGHT).
...
#define KEYD_CYCLEMAP   11   /* PC-FX D button: warp to next baked map (< NUMKEYS) */
#define KEYD_C           12
#define KEYD_X           13
#define KEYD_Y           14
#define KEYD_Z           15
...
// PC-FX mouse turning sensitivity.  MOUSE_TURN_SCALE(s) is the angleturn units
// applied per mouse count; one angleturn unit is 360/65536 deg (it is shifted
// <<16 onto the 32-bit BAM angle), and the real PC-FX mouse reports about 200
// counts per inch.  So the default, sens 6 == 112 units/count, turns 360 deg in
// roughly 3 inches of desk.  Sens 0 == 16 is the port's original hardcoded
// scale, kept selectable for emulator users, whose host mouse feeds far more
// counts per inch than the real one.
#define MOUSE_SENS_MAX          15
#define MOUSE_SENS_DEFAULT      6
#define MOUSE_TURN_SCALE(s)     (16 + (s) * 16)
