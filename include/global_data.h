...
int     mousebuttons;   // current mouse button mask
int     mousex, mousey; // accumulated relative mouse motion, consumed per tic
boolean cycleweapondown;
int     cycleweaponcooldown;   // tics left before another cycle-weapon press can register
boolean cycledowndown;         // edge state for the mouse-mode cycle-down button (X)
boolean cycleupdown;           // edge state for the mouse-mode cycle-up button (Y)
boolean mousepresent;          // latched true once a mouse answers on port 2
int     mouseSensitivity;      // 0..MOUSE_SENS_MAX, see MOUSE_TURN_SCALE()
