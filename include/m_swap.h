...
/* WADs and network packets are little-endian. On a big-endian host the
 * WAD<->host macros must actually swap; on little-endian they stay no-ops.
 * GBADoom shipped these as unconditional no-ops (LE-only) — that was the
 * single biggest source of breakage on a big-endian target, so restore real
 * swaps. PC-FX (V810) is little-endian, so this branch is dead code here but
 * kept for portability of the shared engine core. */
