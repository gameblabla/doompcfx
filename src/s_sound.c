...
void S_StartSoundAtVolume(mobj_t *origin, int sfx_id, int volume)
{
    ...
    // Sounds the player makes, and sounds with no world origin (menu, intermission),
    // play unattenuated -- and get their own backend voice, so the player's weapon
    // and a monster's reaction to it don't cut each other off.
    int is_player = (!origin || origin == _g->player.mo);
    ...
    int h = I_StartSound(sfx_id, cnum, volume, sep, is_player);
    if (h != -1)
    {
...
static boolean S_SoundIsPlaying(int cnum)
{
    ...
    if(channel->sfxinfo)
    {
        ...
        // Inverted upstream (GBADoom returns tickend < ticknow), which made
        // S_UpdateSounds free every channel the tic after it started. Harmless while
        // S_StopChannel was struct-only; now that it silences a real RP2040 voice it
        // would cut every sound to one tic.
...
void S_StopChannel(int cnum)
{
    ...
    if (c->sfxinfo)
    {
        ...
        // silence the backend voice; a retrigger would cut it anyway, but a bare
        // stop (level exit, teleport, monster killed mid-sound) has no replacement
        I_StopSound(cnum);
