...
#ifdef DEV_TIC_PROFILE
  uint64_t _t1 = itu_ticks();
#endif
  P_RunThinkers();
#ifdef DEV_TIC_PROFILE
  uint64_t _t2 = itu_ticks();
#endif
  P_UpdateSpecials();
  P_RespawnSpecials();
#ifdef DEV_TIC_PROFILE
  g_rpa_th_player   += (uint32_t)(_t1 - _t0);
  g_rpa_th_thinkers += (uint32_t)(_t2 - _t1);
  g_rpa_th_specials += (uint32_t)(itu_ticks() - _t2);
#endif
  P_MapEnd();
