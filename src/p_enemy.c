...
typedef enum {
  ...
  DI_SOUTHEAST,
  DI_NODIR,
  DI_NODIRWAIT,   /* stuck: waiting out a cooldown before re-pathing (d32xr fc0f240) */
  NUMDIRS
} dirtype_t;
...
static boolean P_Move(mobj_t *actor, boolean dropoff) /* killough 9/12/98 */
{
  ...
  /* stuck monster serving its re-path cooldown: report "moved" so A_Chase
   * doesn't call the (expensive) P_NewChaseDir again until movecount runs out.
   * Must precede the xspeed[]/yspeed[] indexing below (arrays are size 8). */
...
static void P_DoNewChaseDir(mobj_t *actor, fixed_t deltax, fixed_t deltay)
{
  ...
  if (olddir == DI_NODIRWAIT)         // re-pathing now: treat the wait as no-dir
    olddir = DI_NODIR;
  turnaround = olddir;
  ...
  {
    /* Totally stuck. Rather than re-running this whole search (trig + several
     * P_TryWalk/blockmap passes) every tic, park the monster in a wait state
     * for a short random period first. P_Move short-circuits during the wait,
     * so A_Chase won't re-path until movecount expires. (cf. d32xr fc0f240)
     * P_TryWalk is still invoked in exactly the same cases as before. */
    actor->movedir = DI_NODIRWAIT;
...
void A_VileChase(mobj_t* actor)
{
    ...
    if (actor->movedir != DI_NODIR && actor->movedir != DI_NODIRWAIT)
    {
        // check for corpses to raise (guard DI_NODIRWAIT: xspeed[]/yspeed[] are size 8)
        _g->viletryx =
...
/* misc1 degrees -> BAM angle. (deg<<32)/360 == deg * round(2^32/360); the 32-bit
 * multiply avoids libgcc's __udivdi3 (no 64-bit divide). Error <= ~104 BAM
 * (~9e-6 deg) over the 0..359 range, imperceptible -- and misc1 is unused by
 * vanilla DOOM states anyway. */
void A_Turn(mobj_t *mo)
{
  mo->angle += (unsigned int)((unsigned int)mo->state->misc1 * 11930465u);
}
