...
#include "pcfx_text.h"    /* status bar on the VDC BG-tile layer */
...
{
  // reset the RAM strip to the STBARFX background
  ST_refreshBackground();

  // composite all widgets into the strip at full resolution
  ST_drawWidgets(true);

  // flush the composited 256x32 strip to the bottom KRAM rows
  HUD_BlitStrip();
}
...
void ST_Drawer(boolean statusbaron, boolean refresh)
{
   ...
   */
    ...
    if (statusbaron)
    {
        ...
        // PC-FX: the bar is a VDC tile overlay whose BAT cells are wiped every frame by
        // pcfx_text_begin (D_Display); re-stamp them here so the bar shows even on frames
        // with no widget change. The tile CG is only re-uploaded on a change (below).
        pcfx_text_hud_place();
...
static void ST_loadGraphics(boolean doload)
{
    ...
    for (i=0;i<10;i++)
    {
        // Kippykip's narrow GBA red numbers (10px wide vs stock STTNUM's 14) so
        // the readouts fit the small GBA status bar. Merged into the baked WAD
        // from gbadoom.wad by tools/bake_wad.py (--merge); see Makefile.
        sprintf(namebuf, "STGANUM%d", i);
    ...
    // status bar background: the pre-built 256px-wide raw 8bpp image (240px GBA
    // HUD + 8px black side bars), baked into the WAD as STBARFX. Drawn 1:1.
    _g->stbarbg = (const patch_t *) W_CacheLumpName("STBARFX");
    _g->stbar_len = 256 * ST_HEIGHT;
...
void ST_Start(void)
{
  ...
  ST_createWidgets();
  st_stopped = false;
  /* The PC-FX HUD is resident VDC tile CG, not part of either KING framebuffer;
   * one full composition is enough to initialize it. */
  _g->st_needrefresh = 1;
}
