...
{
    /* PC-FX: the framebuffer is KING KRAM (not a linear RAM buffer we can read
     * back per pixel), and we double-buffer with a hardware page flip. The melt
     * effect needs random-access RMW of two framebuffers, so skip it and cut
     * instantly — the next frame is already rendered directly to KRAM. */
    (void)ticks;
    return 1;   /* done */
