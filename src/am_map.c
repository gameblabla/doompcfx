...
/* Automap coordinates are logical framebuffer words, not physical pixels.
 * SCREENWIDTH already spans the full 256px KING plane (each word is two
 * horizontal pixels); using SCREENWIDTH*2 clipped the right half and shifted
 * the player/map centre offscreen. */
...
( event_t*  ev )
{
    ...
    else if (ev->type == ev_keydown)
    {
        ...
        /* A is always the player's use button.  On the full map, dismiss it
         * and pass the same keydown through to G_Responder so a door/switch can
         * be used in one press.  On the overlay map, pass it through directly. */
        if (ch == key_use)
        {
