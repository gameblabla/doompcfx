...
#include "pcfx_text.h"    /* VDC BG font-tile overlay (replaces framebuffer glyph draw) */
...
{
    // PC-FX: text is drawn on the VDC BG font-tile overlay (platform/pcfx_text.c),
    // NOT into the KING framebuffer, so it composites over the scene by hardware and
    // fades with the scene palette. The tile grid is 8px cells (monospace): the line's
    // pixel origin maps to a cell (x/8, y/8) and each glyph advances one cell.
    int row = l->y / 8;
    int col = l->x / 8;
    for (int i = 0; i < l->len; i++)
    {
        unsigned char c = toupper(l->l[i]);
        if (c == '\n') { col = l->x / 8; row++; continue; }   // multiple lines
        if (c == '\t') { col = col - col % 10 + 10; continue; }
        pcfx_text_putc(col, row, c);            // no-op for space / non-printable
        col++;
    }
}
