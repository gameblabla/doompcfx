...
#include "dstrings.h"
#include "pcfx_cdasset.h"  // CD-streamed finale background (not resident)
#include "pcfx_text.h"     // VDC BG font-tile overlay for finale/cast text
...
// Text-crawl speed in HUNDREDTHS of a tic per character. Kept as scaled ints
// (300 == 3.0, 1 == 0.01) so the finale needs no float -- a single float here
// otherwise drags in libgcc's soft-float routines (__gtsf2, *_sf, load/save_fp)
// for no gain. Threshold math below divides the *100 back out.
#define TEXTSPEED    300   // 3.00 tics/char (original value)     // phares
#define TEXTWAIT     250   // original value                    // phares
#define NEWTEXTSPEED 1     // 0.01 tics/char (original value)    // phares
#define NEWTEXTWAIT  1000  // new value                         // phares
#define TEXTSPEED_SCALE 100
...
/* returns hundredths of a tic per character (see TEXTSPEED above) */
...
//
    ...
    if (!_g->finalestage)
    {
        int speed = Get_TextSpeed();   /* hundredths of a tic per char */
        /* killough 2/28/98: changed to allow acceleration */
        if (_g->finalecount > (int)(strlen(_g->finaletext)*speed)/TEXTSPEED_SCALE +
...
static void F_TextWrite (void)
{
    ...
    { // draw some of the text onto the screen
        ...
        int         count = ((_g->finalecount - 10) * TEXTSPEED_SCALE)/Get_TextSpeed(); // phares
        int         w;
        ...
        for ( ; count ; count-- )
        {
            ...
            // PC-FX: draw onto the VDC font-tile overlay (platform/pcfx_text.c). Layout
            // stays in Doom's 320x200 space, scaled to the 256x240 screen, then to 8px
            // cells; the pen advances a fixed 10 (=8 screen px = 1 cell): monospace.
            pcfx_text_putc((cx * 256 / 320) / 8, (cy * 240 / 200) / 8, toupper(c));
            cx += 10;
        }
    }
}
...
static void F_CastPrint (const char* text) // CPhipps - static, const char*
{
    ...
    while (ch)
    {
        ...
        if (!c)
            break;
        width += 8;   // PC-FX monospace: 8px/cell (matches the tile draw below)
    }

    // draw it -- PC-FX: monospace on the VDC font-tile overlay, centred at cell 15
    // (120/8) on row 18 (144/8).
    cx = 120-width/2;
    ch = text;
    while (ch)
    {
        ...
        if (!c)
            break;
        pcfx_text_putc(cx / 8, 18, toupper(c));   // no-op for space/non-printable
        cx += 8;
    }
}
...
void F_Drawer (void)
{
    ...
    else
    {
        ...
        case 1:
        {
            // The episode-end picture is streamed from CD into the framebuffer
            // (not resident in RAM); fall back to the baked patch otherwise.
            const char *pic = (_g->gamemode == retail) ? "CREDIT" : "HELP2";
            // Raw CD->KRAM DMA, cached per framebuffer buffer (static end screen,
            // double-buffered): the CD is read at most once per buffer, not per frame.
            if (!pcfx_cd_background_dma(pic))
                V_DrawNamePatch(0, 0, 0, pic, CR_DEFAULT, VPT_STRETCH);
            break;
        }
        case 2:
